...
invasion_plot <- function(match_id) {
  ...
  invasion_home <- read_csv(paste0('data/invasion/', match_id, '_home.csv')) %>%
    ...
    mutate(
      ...
      Invasion = round(rollapply(Invasion, width = 220, FUN = mean, fill = NA, align = "right", partial = TRUE), 3)
    )
  ...
  acceleration_home <- read_csv(paste0('data/acceleration/', match_id, '_home.csv')) %>%
    ...
    mutate(
      Time = round(Time / 60, 2),
      Acceleration = round(rollapply(Acceleration, width = 220, FUN = mean, fill = NA, align = "right", partial = TRUE), 3)
    )
  ...
  invasion_plot <- ggplotly(
    ggplot() + 
      geom_line(data = invasion_home, aes(x = Time, y = Invasion), color = home_color) + 
      geom_line(data = invasion_away, aes(x = Time, y = Invasion), color = away_color) + 
  ...
  acceleration_plot <- ggplotly(
    ggplot() + 
      geom_line(data = acceleration_home, aes(x = Time, y = Acceleration), color = home_color) + 
      geom_line(data = acceleration_away, aes(x = Time, y = Acceleration), color = away_color) + 
...
ui <- fluidPage(
  ...
  fluidRow(
    column(12, align = "center", tags$h2(
      ...
      style = "font-weight: bold; text-align: center; margin-bottom: 10px; font-size: 18px;"
    ))
  ),
...
server <- function(input, output, session) {
  ...
  match_title <- reactive({
    ...
    get_match_label(matches, selected_match_id())
  })
